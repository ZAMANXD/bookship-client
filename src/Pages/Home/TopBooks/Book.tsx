...
const Book = ({ ...book }) => {
  ...
  return (
    // bg-[#ffffff49]
    <div className="bookCont bg-gradient-to-r from-gray-100 to-white rounded-lg hover:shadow-2xl hover:shadow-gray-400 shadow-lg shadow-gray-300 ease-in-out duration-200 p-5 mx-2 border">
      <div className="relative">
        <Link to={`book/${_id}`}>
          <img
            className={`h-42 lg:h-52 w-auto mx-auto rounded-lg hover:shadow-lg hover:shadow-gray-500 ease-in-out duration-200`}
            src={bookCover}
            alt=""
          />
        </Link>
      ...
      <div className="mt-2 h-24 lg:h-32 relative">
        ...
        <h2 className="text-xs lg:text-md font-semibold text-[#34315D] uppercase">
        ...
        <div className="absolute bottom-0 w-full">
          <div className="flex justify-between items-center lg:my-2">
            ...
            <div className="flex justify-between items-center">
              {[...Array(parseInt(bookRating || 4))].map((star, i) => (
                <AiFillStar key={i} className="text-sm text-yellow-500" />
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-between items-center">
            <div>
              <p className="text-md lg:text-lg font-bold text-[#342D65]">
                $ {discountedPrice}
              </p>
            </div>
